...
function create_new_account($account_name, $code_type_id){
    ...
    $select_type_name = sprintf("select name from types where id='%d'", intval($code_type_id));
    ...
    if($type_name){
      ...
      $insert_code_for_account = sprintf("INSERT INTO `accounts`(`accountName`,`code`) value('%s','%s')", mysql_real_escape_string($account_name), $uniq_code);
