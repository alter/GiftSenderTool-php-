...
class HessDateTime{
	 ...
	 **/
	function HessDateTime($date='now'){
		//echo $date;
		if(is_string($date)) {
		...
		elseif(is_object($date) && is_a($date,'HessDateTime'))
	...
	function &asTime(){
		return new HessDateTime($this->getTime());
	}

	function &asDate() {
		return new HessDateTime($this->getDate());
	}
	...
	function isBefore($time){
		if(HessDateTime::compare($this,$time) == -1)
			return true;
		return false;
	}
	...
	function equals($time){
		if(HessDateTime::compare($this,$time) == 0)
			return true;
		return false;
	}
	...
	function compare($time1,$time2){
		...
		$ts1 = HessDateTime::findTimestamp($time1);
		$ts2 = HessDateTime::findTimestamp($time2);
	...
	function monthName($value=null){
		...
		}elseif(is_object($value) && is_a($value,'HessDateTime'))
			$ts = $value->_timestamp;
		else {
	...
	function findTimestamp($dateObj){
		if(is_object($dateObj) && is_a($dateObj,'HessDateTime'))
